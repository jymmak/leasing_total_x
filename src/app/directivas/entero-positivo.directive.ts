...
export class EnteroPositivoDirective {
  ...
  @HostListener('keydown', ['$event']) onKeyDown(event) {
    ...
    if (this.teclas_numero.indexOf(e.keyCode) !== -1 && length >= this.maxlength) {
      e.preventDefault();
    }
  ...
  @HostListener('keyup', ['$event']) onkeyUp(event) {
    ...
    if (numero.length > this.maxlength) {
      numero = numero.substring(0, this.maxlength);
      // this.valueEntero.emit(parseInt(numero, 0));
    }
  ...
  @HostListener('change', ['$event']) onchange(event) {
    ...
    if (numero.length > this.maxlength) {
      numero = numero.substring(0, this.maxlength);
      this.valueEntero.emit(this.validarNumeros(numero));
    }
